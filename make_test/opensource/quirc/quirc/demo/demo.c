...
static unsigned char *fffff = NULL;
...
static void draw_qr(SDL_Surface *screen, struct quirc *q, struct dthash *dt)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (err) {
			...
			fwrite( fffff, 1,  video_width*video_height, file);
			fclose(file);
		     ...
		     struct quirc *q, struct mjpeg_decoder *mj)
{
	...
	fffff = (unsigned char *)malloc(parms->width * parms->height);
...
static int run_demo(void)
{
	...
	printf("parms->width:%d parms->height:%d\n", parms->width, parms->height);
