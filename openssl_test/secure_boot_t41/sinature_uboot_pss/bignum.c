...
static u32 __cache_data *cache_space = NULL;

void cache_init()
{
	cache_space = malloc(sizeof(char) * 4 * 1024);
	cache_space = memset(cache_space, 0x0f, 4 * 1024);
}

static int cache_space_pos = 0;
void * __cache_text rsa_malloc(int size){
...
void __cache_text rsa_free(void *d)
{
	u32 *v = (u32*)d;
	u32 size = v[-1];
	cache_space_pos -= (size + 3) / 4 + 1;
...
void __cache_text bn_assign(bn_t *a, bn_t *b, u32 digits)
{
	u32 i;
	for(i=0; i<digits; i++) {
		a[i] = b[i];
...
void __cache_text bn_assign_zero(bn_t *a, u32 digits)
{
	u32 i;
	for(i=0; i<digits; i++) {
		a[i] = 0;
...
bn_t __cache_text bn_add(bn_t *a, bn_t *b, bn_t *c, u32 digits)
{
	bn_t ai, carry;
	u32 i;
...
bn_t __cache_text bn_sub(bn_t *a, bn_t *b, bn_t *c, u32 digits)
{
	bn_t ai, borrow;
	u32 i;
...
void __cache_text bn_mul(bn_t *a, bn_t *b, bn_t *c, u32 digits)
{
	bn_t *t;
...
bn_t __cache_text __div64_32(dbn_t *n, bn_t base)
{
	dbn_t rem = *n;
	dbn_t b = base;
	dbn_t res, d = 1;
...
void __cache_text bn_div(bn_t *a, bn_t *b, bn_t *c, u32 cdigits, bn_t *d, u32 ddigits)
{
	dbn_t tmp;
	bn_t ai, t;
	bn_t *cc, *dd;
...
bn_t __cache_text bn_shift_l(bn_t *a, bn_t *b, u32 c, u32 digits)
{
	bn_t bi, carry;
	u32 i, t;
...
bn_t __cache_text bn_shift_r(bn_t *a, bn_t *b, u32 c, u32 digits)
{
	bn_t bi, carry;
	int i;
	u32 t;
...
void __cache_text bn_mod(bn_t *a, bn_t *b, u32 bdigits, bn_t *c, u32 cdigits)
{
	bn_t *t;
	t = (bn_t *)rsa_malloc(BN_MAX_DIGITS * sizeof(bn_t) * 2);
...
void __cache_text bn_mod_mul(bn_t *a, bn_t *b, bn_t *c, bn_t *d, u32 digits)
{
	bn_t *t;
	t = (bn_t *)rsa_malloc(BN_MAX_DIGITS * sizeof(bn_t) * 2);
...
void __cache_text bn_mod_exp(bn_t *a, bn_t *b, bn_t *c, u32 cdigits, bn_t *d, u32 ddigits)
{
	bn_t ci;
	bn_t *bpower[3],*t;
	int i;
...
int __cache_text bn_cmp(bn_t *a, bn_t *b, u32 digits)
{
	int i;
...
u32 __cache_text bn_digits(bn_t *a, u32 digits)
{
	int i;
	for(i=digits-1; i>=0; i--) {
		if(a[i])    break;
...
static bn_t __cache_text bn_sub_digit_mul(bn_t *a, bn_t *b, bn_t c, bn_t *d, u32 digits)
{
	dbn_t result;
	bn_t borrow, rh, rl;
	u32 i;
...
static u32 __cache_text bn_digit_bits(bn_t a)
{
	u32 i;
